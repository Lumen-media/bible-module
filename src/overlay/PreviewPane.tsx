    ...
    >
      ...
      <div className="relative z-10 flex h-full w-full items-center gap-1 px-2.5">
        <div className="flex min-w-0 flex-1 flex-col items-start leading-tight">
          <span className="truncate text-[11px] text-white/80">{bookLabel}</span>
          <span className="truncate text-[10px] text-white/50">{versionLabel}</span>
        </div>
        <div className="h-7 w-px shrink-0 bg-white/20" />
        <span className="shrink-0 text-lg font-semibold text-white/90">{verseNumber ?? '1'}</span>
      </div>
